...
| --- | --- | --- | --- |
| `KycService.review()` writes `reviewedBy` but never `reviewedAt` | [api/src/kyc/kyc.service.ts:48-59](api/src/kyc/kyc.service.ts#L48-L59) | A reviewed KYC case is attributable to a reviewer but not to a time. The column exists in the schema and `list()` selects it, so the admin console reads a field that is permanently `null`. | Open |
